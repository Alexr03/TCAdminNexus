...
namespace TCAdminNexus.Commands.Admin
{
    ...
    public class NodeCommands : BaseCommandModule
    {
        [Command("CMD")]
        public async System.Threading.Tasks.Task ServerCmd(CommandContext ctx, string serverName, [RemainingText] string script)
        {
        ...
        [Command("PowerShell")]
        public async System.Threading.Tasks.Task ServerPowerShell(CommandContext ctx, string serverName, [RemainingText] string script)
        {
            await ctx.TriggerTypingAsync();
        ...
        public async System.Threading.Tasks.Task RunAllPowerShell(CommandContext ctx, [RemainingText] string script)
        {
            if (!script.StartsWith("```\n") && !script.EndsWith("```"))
            {
                await ctx.RespondAsync(embed: EmbedTemplates.CreateErrorEmbed("PowerShell",
        ...
        public async System.Threading.Tasks.Task RestartMonitor(CommandContext ctx, string serverName)
        {
            if (!(Server.GetServers(true, serverName)[0] is Server server))
            {
                await ctx.RespondAsync(embed: EmbedTemplates.CreateErrorEmbed("Server Not Found",
        ...
        public async System.Threading.Tasks.Task RestartMonitor(CommandContext ctx)
        {
            foreach (Server enabledServer in Server.GetEnabledServers())
            {
                if (enabledServer.IsMaster) continue;
