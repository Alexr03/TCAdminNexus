...
namespace TCAdminNexus.Objects.Emulators
{
    ...
    {
        private readonly LocalConfiguration<ServiceMenuConfiguration> _configuration =
            new LocalConfiguration<ServiceMenuConfiguration>("ServiceMenuConfig");

        private readonly string[] _sizeSuffixes = {"bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"};

        private IEnumerable<ServiceMenuModule> NexusServiceMenuModules
        {
            get
            {
                var modules = typeof(ServiceMenuModule)
                    .Assembly.GetTypes()
                    .Where(t => t.IsSubclassOf(typeof(ServiceMenuModule)) && !t.IsAbstract)
                    .Select(t => (ServiceMenuModule) Activator.CreateInstance(t));
                return modules;
            }
        }
...
#pragma warning restore 4014
        ...
        private string GenerateActions(DiscordClient client, IEnumerable<ServiceMenuModule> modules)
        {
